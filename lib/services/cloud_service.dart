...
class CloudService extends GetxService {
  ...
  String get _provider =>
      _hive.getSetting(AppConstants.keyCloudProvider, defaultValue: 'freeai') ??
      'freeai';

  String get _apiKey {
    switch (_provider) {
      case 'freeai':
        return ''; // keyless anonymous tier
      case 'anthropic':
  ...
  String get _model {
    switch (_provider) {
      case 'freeai':
        return _hive.getSetting(AppConstants.keyFreeAiModel,
  ...
  bool get isConfigured {
    if (_provider == 'freeai') return true; // keyless — works out of the box
    if (_provider == 'custom') {
      final baseUrl =
  ...
  String get _openAICompatibleEndpoint {
    switch (_provider) {
      case 'freeai':
        return AppConstants.freeAiEndpoint;
      case 'nvidia':
