...
class SettingsController extends GetxController {
  ...
  final cloudProvider = 'freeai'.obs;
  final openaiKey = ''.obs;
  ...
  final freeAiModel = 'openai-fast'.obs;
  ...
  final freeAiModelController = TextEditingController();
  ...
  void _loadSettings() {
    ...
    cloudProvider.value = _hive.getSetting(AppConstants.keyCloudProvider,
            defaultValue: 'freeai') ??
        'freeai';
    freeAiModel.value = _hive.getSetting(AppConstants.keyFreeAiModel,
            defaultValue: 'openai-fast') ??
        'openai-fast';
  ...
  void _syncControllers() {
    ...
    freeAiModelController.text = freeAiModel.value;
  }
  ...
  TextEditingController apiKeyControllerFor(String provider) {
    switch (provider) {
      ...
      case 'freeai':
        return openaiKeyController; // keyless — never shown as a key field
      default:
  ...
  /// Re-reads every setting from Hive and syncs the text controllers.
  /// Used after a cloud restore or on app startup.
  void reloadSettings() {
    _loadSettings();
    _syncControllers();
  }
  ...
  /// Fires an automatic cloud backup whenever an API key or model changes.
  void _notifyKeyBackup() {
    if (Get.isRegistered<KeyBackupService>()) {
      unawaited(Get.find<KeyBackupService>().syncNow());
    }
  }
  ...
  Future<void> setCloudModel(String provider, String model) async {
    switch (provider) {
      ...
      case 'custom':
        ...
        break;
      case 'freeai':
        freeAiModel.value = model;
        freeAiModelController.text = model;
        await _hive.setSetting(AppConstants.keyFreeAiModel, model);
        break;
    }
    _notifyKeyBackup();
  }
