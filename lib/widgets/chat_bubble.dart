              ...
              Row(
                ...
                children: [
                  if (!isUser && answerContent.isNotEmpty) ...[
                    _SpeakButton(text: answerContent, isDark: isDark),
...
/// Text-to-speech "speak" button for assistant replies.
class _SpeakButton extends StatefulWidget {
  final String text;
  final bool isDark;

  const _SpeakButton({required this.text, required this.isDark});
...
class _SpeakButtonState extends State<_SpeakButton> {
  ...
  Future<void> _toggle() async {
    ...
    try {
      ...
      final plain = _stripMarkdown(widget.text);
  ...
  @override
  Widget build(BuildContext context) {
    final hint = Theme.of(context).hintColor.withValues(alpha: 0.6);
    return InkWell(
      borderRadius: BorderRadius.circular(10),
      onTap: _toggle,
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Icon(
              ...
              size: 13,
              color: _speaking ? const Color(0xFF0A84FF) : hint,
            ),
          ],
        ),
      ),
