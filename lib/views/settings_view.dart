...
class SettingsView extends GetView<SettingsController> {
    ...
    return Scaffold(
      ...
      body: Obx(() => ListView(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              _sectionLabel(context, 'CLOUD KEY BACKUP'),
              _buildKeyBackupCard(context, isDark),
              const SizedBox(height: 24),
  ...
  // ── Cloud key backup ──────────────────────────────
  Widget _buildKeyBackupCard(BuildContext context, bool isDark) {
    final backup = Get.find<KeyBackupService>();
    return Obx(() {
      final configured = backup.isConfigured;
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          configured
                              ? 'Every API key + provider/model syncs to '
                                  'your Supabase project automatically.'
                              : 'Back up every API key to your own Supabase '
                                  'project so you never lose them.',
                          style: GoogleFonts.inter(
                              fontSize: 12,
                              color: Theme.of(context).hintColor),
                        ),
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () => _showBackupConfigDialog(context, backup),
                      icon: Icon(configured ? Icons.edit_outlined : Icons.add,
                          size: 18),
                      label: Text(configured ? 'Change' : 'Set up'),
                    ),
                  ),
                  if (configured) ...[
                        ...
                        onPressed: () async {
                          ...
                          if (ok) {
                            ...
                            Get.snackbar('Restore complete',
                                'All API keys and settings were restored.',
                                snackPosition: SnackPosition.BOTTOM);
                          } else {
                            Get.snackbar('Nothing to restore',
                                'No backup found for this passcode.',
                                snackPosition: SnackPosition.BOTTOM);
                          }
                        },
  ...
  Future<void> _showBackupConfigDialog(
      BuildContext context, KeyBackupService backup) async {
    final urlController = TextEditingController(
        text: Get.find<HiveService>().getSetting<String>(
                KeyBackupService.backupUrlKey) ??
            '');
    final keyController = TextEditingController(
        text: Get.find<HiveService>().getSetting<String>(
                KeyBackupService.backupAnonKey) ??
            '');
    final passController = TextEditingController(
        text: Get.find<HiveService>().getSetting<String>(
                KeyBackupService.backupPasscode) ??
            '');

    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Cloud key backup'),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: const Text(
                  'create table nova_key_backup (\n'
                  '  passcode text primary key,\n'
                  '  payload text,\n'
                  '  updated_at timestamptz\n'
                  ');\n'
                  'alter table nova_key_backup enable row level security;',
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              final anon = keyController.text.trim();
              final pass = passController.text.trim();
              if (url.isEmpty || anon.isEmpty || pass.isEmpty) {
                Get.snackbar('Missing fields',
                    'Project URL, anon key and passcode are required.',
                    snackPosition: SnackPosition.BOTTOM);
                return;
              }
              await backup.configure(
                  url: url, anonKey: anon, passcode: pass);
              if (ctx.mounted) Navigator.pop(ctx);
              Get.snackbar('Backup configured',
                  'All API keys are now synced automatically.',
                  snackPosition: SnackPosition.BOTTOM);
            },
